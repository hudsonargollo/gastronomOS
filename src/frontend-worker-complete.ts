...
app.get('*', async (c) => {
  try {
    ...
    // Handle dashboard and other app routes - serve the complete Next.js pages
    if (pathname.startsWith('/dashboard') || 
      ...
      // Try to serve the actual Next.js built page first
      const staticContent = c.env.__STATIC_CONTENT as KVNamespace;
      if (staticContent) {
        try {
          const manifestString = (c.env.__STATIC_CONTENT_MANIFEST as string) || '{}';
      ...
      // Fallback to complete dashboard if Next.js files not found
      return new Response(buildCompleteDashboard(), {
        headers: {
          'Content-Type': 'text/html; charset=utf-8',
          'Cache-Control': 'public, max-age=3600',
        },
...
function buildCompleteDashboard(): string {
  ...
  parts.push('<meta charSet="utf-8"/>');
  parts.push('<meta name="viewport" content="width=device-width, initial-scale=1"/>');
  parts.push('<link rel="preload" href="/_next/static/media/83afe278b6a6bb3c-s.p.3a6ba036.woff2" as="font" crossorigin="" type="font/woff2"/>');
  parts.push('<link rel="stylesheet" href="/_next/static/chunks/d75cdfeb2cebbca0.css" data-precedence="next"/>');
  parts.push('<link rel="preload" as="script" fetchPriority="low" href="/_next/static/chunks/43c09a0f3b518e18.js"/>');
  
  // All the original script tags
  const scripts = [
    '0f94adcdd45354d1.js',
    '59b0cdc1cb5e9a3e.js', 
    '964bb40aeb50325b.js',
    'turbopack-921c773a0b302f4c.js',
    '91f1628386242910.js',
    'aae562da308fa0ed.js',
    '1fa725e0a787855e.js',
    '49e614a8b39e78e9.js',
  ...
  scripts.forEach(script => {
    parts.push(`<script src="/_next/static/chunks/${script}" async=""></script>`);
  });
  
  parts.push('<meta name="next-size-adjust" content=""/>');
  parts.push('<title>GastronomOS - O Sistema Operacional do seu Restaurante</title>');
  parts.push('<meta name="description" content="O Sistema Operacional completo do seu restaurante para estoque, compras, transferências e análises"/>');
  parts.push('<meta name="keywords" content="restaurante,gestão,estoque,PDV,gastronomia"/>');
  parts.push('<link rel="icon" href="/favicon.ico?favicon.0b3bf435.ico" sizes="256x256" type="image/x-icon"/>');
  parts.push('<script src="/_next/static/chunks/a6dad97d9634a72d.js" noModule=""></script>');
  parts.push('</head>');
  ...
  // Sidebar
  parts.push(buildSidebar());
  
  // Main content area
  parts.push(buildMainContent());
...
function buildSidebar(): string {
  ...
  parts.push('<div class="relative" style="opacity:1;transform:none">');
  ...
  // Navigation items
  const navItems = [
    { name: 'Painel', href: '/dashboard/', icon: 'house', active: true },
    { name: 'Estoque', href: '#', icon: 'package', hasChevron: true },
    { name: 'Compras', href: '#', icon: 'shopping-cart', hasChevron: true },
    { name: 'Transferências', href: '#', icon: 'arrow-right-left', hasChevron: true },
    { name: 'Alocações', href: '#', icon: 'allocations', hasChevron: true },
    { name: 'Análises', href: '#', icon: 'chart-column', hasChevron: true },
  ...
  navItems.forEach(item => {
    parts.push('<div><div tabindex="0">');
    
    const activeClass = item.active 
      ? 'bg-gradient-to-r from-orange-500/20 to-red-500/20 text-orange-300 shadow-lg shadow-orange-500/10'
      : 'text-slate-300 hover:bg-slate-700/50 hover:text-white';
    
    parts.push(`<a class="group flex items-center rounded-xl px-3 py-2.5 sm:py-3 text-sm font-medium transition-all duration-200 relative min-h-[44px] ${activeClass}" href="${item.href}">`);
    
    // Add icon
    parts.push(getNavigationIcon(item.icon, item.active));
    
    parts.push(`<span class="ml-3 flex-1 truncate">${item.name}</span>`);
    
    // Add chevron if needed
    if (item.hasChevron) {
      parts.push('<div class="ml-auto">');
  ...
  parts.push('<span class="text-sm font-semibold text-white">JD</span>');
  parts.push('</div>');
...
function buildMainContent(): string {
  ...
  // Header with Framer Motion style
  parts.push('<header class="flex h-14 sm:h-16 items-center justify-between border-b border-slate-200 bg-white/80 backdrop-blur-sm px-3 sm:px-6 shadow-sm" style="opacity:0;transform:translateY(-20px)">');
  
  // Left side of header
  parts.push('<div class="flex items-center space-x-2 sm:space-x-4 flex-1 min-w-0">');
  
  // Mobile menu button
  parts.push('<button data-slot="button" data-variant="ghost" data-size="sm" class="inline-flex items-center justify-center whitespace-nowrap text-sm font-medium transition-all disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg:not([class*=\'size-\'])]:size-4 [&_svg]:shrink-0 outline-none focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive hover:bg-accent hover:text-accent-foreground dark:hover:bg-accent/50 rounded-md gap-1.5 has-[>svg]:px-2.5 lg:hidden h-9 w-9 p-0 shrink-0">');
  parts.push('<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-menu h-5 w-5" aria-hidden="true">');
  ...
  // Page title
  parts.push('<div class="min-w-0 flex-1">');
  parts.push('<h1 class="text-lg sm:text-xl lg:text-2xl font-bold text-slate-900 truncate">Painel</h1>');
  parts.push('<p class="text-xs sm:text-sm text-slate-500 hidden sm:block truncate">Bem-vindo de volta, gerencie suas operações do restaurante</p>');
  parts.push('</div>');
  parts.push('</div>');
  ...
  parts.push('<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-search absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" aria-hidden="true">');
  parts.push('<path d="m21 21-4.34-4.34"></path><circle cx="11" cy="11" r="8"></circle>');
  parts.push('</svg>');
  parts.push('<input data-slot="input" class="file:text-foreground placeholder:text-muted-foreground selection:bg-primary selection:text-primary-foreground dark:bg-input/30 h-9 w-full min-w-0 rounded-md border px-3 py-1 text-base shadow-xs outline-none file:inline-flex file:h-7 file:border-0 file:bg-transparent file:text-sm file:font-medium disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50 md:text-sm focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive pl-10 bg-slate-50 border-slate-200 focus:bg-white transition-colors" placeholder="Buscar produtos, pedidos, transferências..."/>');
  parts.push('</div>');
  parts.push('</div>');
  ...
  parts.push('<div class="h-full" style="opacity:0;transform:translateY(20px)">');
...
function buildWelcomeBanner(): string {
  ...
  parts.push('<div class="bg-gradient-to-r from-orange-500 via-red-500 to-pink-500 rounded-2xl p-4 sm:p-6 lg:p-8 text-white relative overflow-hidden" style="opacity:0;transform:translateY(20px)">');
...
function buildStatsGrid(): string {
  ...
  parts.push('<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 lg:gap-6">');
