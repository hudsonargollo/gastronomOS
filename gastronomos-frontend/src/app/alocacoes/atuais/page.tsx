...
const currentAllocations = [
  {
    ...
    totalQuantity: 150,
    allocations: [
      { location: 'COZINHA', quantity: 60, percentage: 40 },
      { location: 'ESTOQUE', quantity: 90, percentage: 60 }
    ],
    status: 'active',
  ...
  },
  {
    ...
    totalQuantity: 80,
    allocations: [
      { location: 'COZINHA', quantity: 40, percentage: 50 },
      { location: 'ESTOQUE', quantity: 40, percentage: 50 }
    ],
    status: 'active',
...
export default function CurrentAllocationsPage() {
  return (
    <MainLayout title="Alocações Atuais">
            ...
            <CardContent className="p-6">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-600">Alocações Ativas</p>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardContent className="p-6">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <p className="text-2xl font-bold text-slate-900">2</p>
                  <p className="text-sm text-slate-600">Locais</p>
                </div>
              </div>
            </CardContent>
          </Card>
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-slate-600">
                          Total: {allocation.totalQuantity} unidades • Atualizado: {allocation.lastUpdated}
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center space-x-2">
                      <Badge className="bg-green-100 text-green-700">
                        {allocation.status === 'active' ? 'ATIVO' : allocation.status.toUpperCase()}
                      </Badge>
                ...
                <CardContent className="space-y-4">
                      ...
                      <div key={allocIndex} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-slate-600">{alloc.quantity} unidades</p>
                          </div>
                        </div>
                  ...
                  <div className="pt-4 border-t border-slate-100">
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-slate-600">Distribuição de Alocação</span>
                      <span className="font-medium text-slate-900">
                        {allocation.allocations.reduce((sum, alloc) => sum + alloc.percentage, 0)}% Alocado
                      </span>
                    </div>
                      ...
                      <div className="flex h-3 rounded-full overflow-hidden">
                        {allocation.allocations.map((alloc, allocIndex) => (
                          <div
                            key={allocIndex}
                            className={`h-3 ${
                              allocIndex === 0 ? 'bg-blue-500' : 'bg-green-500'
                            }`}
                            style={{ width: `${alloc.percentage}%` }}
                          />
                        ))}
