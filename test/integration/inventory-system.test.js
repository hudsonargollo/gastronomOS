...
describe('Inventory System Integration Tests', () => {
    ...
    describe('Stock Alert Generation and Availability Updates', () => {
        ...
        it('should broadcast inventory availability update via WebSocket', async () => {
            ...
            const result = await mockWebSocketService.broadcastInventoryUpdate(testTenantId, menuItemId, false, 'Insufficient ingredients');
            expect(result.success).toBe(true);
        ...
        it('should update menu item availability based on ingredient stock', async () => {
            ...
            const result = await recipeEngine.checkMenuItemAvailability([menuItemId], testTenantId, testLocationId);
    ...
    describe('Concurrent Inventory Operations', () => {
        ...
        it('should handle reservation conflicts with retry', async () => {
            ...
            const firstAttempt = await mockInventoryService.reserveInventoryForTransfer('prod-1', testLocationId, 4, 'order_retry_001', 'user-001', testTenantId).catch(err => ({ success: false, error: err.message }));
            expect(firstAttempt.success).toBe(false);
            // Retry should succeed
            const retryResult = await mockInventoryService.reserveInventoryForTransfer('prod-1', testLocationId, 4, 'order_retry_001', 'user-001', testTenantId);
