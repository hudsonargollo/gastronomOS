...
import type { Order, OrderState, Payment, PaymentMethod, PaymentStatus } from '../../src/db/schema';
...
describe('Multi-Interface Workflow Integration Tests', () => {
  ...
  describe('Real-Time Synchronization Between Interfaces', () => {
    it('should synchronize order state changes across all connected interfaces', async () => {
      ...
      const broadcastResult = await mockWebSocketService.broadcastOrderStateChange(
        testTenantId,
        orderId,
        newState,
        { id: orderId, state: newState, version: 2 }
      );
    ...
    it('should update menu availability in real-time when inventory changes', async () => {
      ...
      const broadcastResult = await mockWebSocketService.broadcastInventoryUpdate(
        testTenantId,
        menuItemId,
      ...
      expect(broadcastResult.success).toBe(true);
      expect(mockWebSocketService.broadcastInventoryUpdate).toHaveBeenCalledWith(
        testTenantId,
        menuItemId,
        false,
        'Insufficient ingredients'
    ...
    it('should synchronize payment status updates across interfaces', async () => {
      ...
      const broadcastResult = await mockWebSocketService.broadcastPaymentStatusUpdate(
        testTenantId,
        orderId,
        paymentId,
        'COMPLETED'
  ...
  describe('Payment Processing End-to-End Workflows', () => {
    ...
    it('should handle manual payment logging for external machines', async () => {
      const manualPaymentData = {
        ...
        tenantId: testTenantId,
        method: 'MANUAL_CARD' as PaymentMethod,
        amount: 2800,
  ...
  describe('Error Handling and Recovery', () => {
    ...
    it('should handle inventory service unavailability', async () => {
      ...
      const result = await mockInventoryService.checkInventoryAvailability(
        'product-001',
        testLocationId,
        10
      ).catch(err => ({
