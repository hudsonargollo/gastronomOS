...
import { PaymentGatewayService, type PaymentProcessingRequest, type WebhookNotification } from '../../src/services/payment-gateway';
...
describe('Payment Gateway Integration Tests', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    paymentService = new PaymentGatewayService(mockDb as any, 'test-encryption-key');
  });
  ...
  describe('Error Scenarios and Fallback Mechanisms', () => {
    ...
    it('should handle rate limiting (429) with exponential backoff', async () => {
      ...
      mockFetch.mockResolvedValueOnce({
        ...
        status: 429,
        headers: {
          get: (header: string) => header === 'X-RateLimit-Reset' ? '60' : null
        },
        json: () => Promise.resolve({
  ...
  describe('Payment Gateway Configuration', () => {
    it('should validate credentials during configuration', async () => {
      ...
      const result = await paymentService.configureGateway(
        testTenantId,
        'MERCADO_PAGO',
        testAccessToken,
        testPublicKey,
